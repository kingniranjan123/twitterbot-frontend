...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { name: 'Extracted Tweets', href: '/extracted', icon: <FileDown size={20} /> },
      ...
      <nav className="flex-1 px-4 space-y-1 mt-2 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`${isActive ? 'text-purple-400' : 'group-hover:text-pink-400 transition-colors'} flex-shrink-0`}>
                {item.icon}
              </span>
              <span className="font-medium text-sm">{item.name}</span>
              {item.badge === 'held' && (
                <span className="ml-auto w-2 h-2 rounded-full bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.8)]" />
              )}
            </Link>
          );
        })}
